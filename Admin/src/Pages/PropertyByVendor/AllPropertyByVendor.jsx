...
const AllPropertyByVendor = () => {
    ...
    useEffect(() => {
        const fetchUsers = async () => {
            try {
                const res = await axios.get(`https://api.aashreyrealtors.com/api/v1/all-users`);
    ...
    const fetchPropertiesByUser = async (userId, filterStatus = statusFilter) => {
        try {
            const res = await axios.get(`https://api.aashreyrealtors.com/api/v1/get-properties-by-vendor/${userId}`);
    ...
    const handleDelete = async (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    await axios.delete(`https://api.aashreyrealtors.com/api/v1/delete-property/${id}`);
    ...
    const handleStatusChange = async (id, status) => {
        try {
            await axios.patch(`https://api.aashreyrealtors.com/api/v1/properties/${id}/status`, { status });
